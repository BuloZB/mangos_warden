...
void Warden::Init(WorldSession *pClient, BigNumber *K)
{
    ...
    */
    ...
    sLog.outWarden("Requesting status of this module from the client...");
    RequestModule();
...
void WorldSession::HandleWardenDataOpcode(WorldPacket & recv_data)
{
    ...
    uint8 Opcode;
    recv_data >> Opcode;
    sLog.outWarden("Got packet, %02X, size %u.", Opcode, recv_data.size() - 1);
    ...
    switch(Opcode)
    {
        ...
        case WARDEN_CMSG_HASH_REQUEST_REPLY:
            ...
            break;
        default:
            sLog.outWarden("Got unknown opcode %02X of size %u.", Opcode, recv_data.size() - 1);
            break;
    }
}
...
{
    sLog.outWarden("SendModuleToClient");
...
void Warden::RequestHash()
{
    ...
    Client->SendPacket(&pkt);
    sLog.outWarden("Request hash send");
}
...
void Warden::RequestHashReply(ByteBuffer &buff)
{
    ...
    {
        sLog.outWarden("0x04 isn't equal");
...
void Warden::RequestData()
{
    ...
    for (std::vector<uint32>::iterator itr = SendDataId.begin(); itr != SendDataId.end(); ++itr)
        sLog.outWarden("SendDataId send check id %u", *itr);
    ...
    pkt.append(buff);
    Client->SendPacket(&pkt);
    sLog.outWarden("Request data send");
